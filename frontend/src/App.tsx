...
function SettingsModal({ onSave, onClose }: SettingsModalProps) {
  return <div className="modal-backdrop" role="presentation"><form className="settings-modal" aria-modal="true" aria-labelledby="settings-title" onSubmit={onSave}><div className="settings-modal__heading"><div><p className="section-label">Settings</p><h2 id="settings-title">Operational configuration</h2></div><button type="button" className="modal-close" onClick={onClose} aria-label="Close settings">&times;</button></div><p className="settings-copy">Update solver and backend preferences for the operations workspace.</p><div className="settings-fields"><label>Default solver<select defaultValue="QAOA+" aria-label="Default solver"><option>QUBO</option><option>QAOA</option><option>QAOA+</option><option>FALCON</option></select></label><label>Number of layers<input defaultValue="5" inputMode="numeric" /></label><label>Quantum backend<select defaultValue="AerSimulator" aria-label="Quantum backend"><option>AerSimulator</option><option>IBM QPU</option></select></label><label>Parameter alpha<input defaultValue="0.73" inputMode="decimal" /></label><label>Parameter beta<input defaultValue="0.27" inputMode="decimal" /></label></div><div className="settings-actions"><button type="submit" className="save-settings">Save Settings</button><button type="button" className="cancel-settings" onClick={onClose}>Cancel</button></div></form></div>
}
