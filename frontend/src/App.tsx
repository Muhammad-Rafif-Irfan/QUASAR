...
function OptimizationLoading({ pipelineStatus }: { pipelineStatus: string }) {
  return <div className="optimization-overlay" role="status" aria-live="polite"><div><Route size={20} /><strong>Optimizing routes...</strong><span>{pipelineStatus}</span></div></div>
}
...
function App() {
  ...
  const [pipelineStatus, setPipelineStatus] = useState('Preparing quantum-classical hybrid pipeline...')
  ...
  // Real optimization: call backend API, with graceful fallback
  const runOptimization = useCallback(() => {
    setIsOptimizing(true)
    setPipelineStatus('Connecting to QUASAR quantum-classical pipeline...')

    const stopsPayload = stops.map((s) => ({ name: s.name, lat: s.lat, lon: s.lon }))
    const depotPayload = { name: depot.name, lat: depot.lat, lon: depot.lon }

    // Fallback timer: if backend doesn't respond within 8s, proceed with local routes
    const fallbackTimer = window.setTimeout(() => {
      console.log('[QUASAR] Backend timeout — using local greedy routing as fallback')
      setPipelineStatus('Using local routing (backend unavailable)')
    ...
    fetch(`${API_BASE}/api/v1/optimize`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ depot: depotPayload, stops: stopsPayload }),
    })
      .then((r) => {
      ...
      .then((data) => {
        setPipelineStatus(`Pipeline started — run ${data.run_id.slice(0, 8)}...`)

        // Poll for completion
        pollRef.current = setInterval(() => {
          fetch(`${API_BASE}/api/v1/optimize/${data.run_id}`)
            .then((r) => r.json())
            .then((status) => {
              if (status.status === 'RUNNING') {
                setPipelineStatus(`Running quantum solver... (${status.results?.length || 0}/3 algorithms completed)`)
              } else if (status.status === 'COMPLETED') {
                clearTimeout(fallbackTimer)
                if (pollRef.current) clearInterval(pollRef.current)

                // Log results to console for debugging
                const algos = (status.results || []).map((r: { algorithm: string; distance_meters: number; approximation_ratio: number }) =>
                  `${r.algorithm}: ${r.distance_meters.toFixed(0)}m (ratio=${r.approximation_ratio?.toFixed(3) || 'N/A'})`
                )
                console.log(`[QUASAR] Optimization complete:`, algos)

                setPipelineStatus('Optimization complete — loading results...')
                window.setTimeout(() => {
                  setIsOptimizing(false)
                  setCurrentScreen('results')
                }, 600)
                ...
                clearTimeout(fallbackTimer)
                if (pollRef.current) clearInterval(pollRef.current)
                console.warn('[QUASAR] Backend optimization failed:', status.error_message)
                setPipelineStatus('Backend failed — using local routing')
                window.setTimeout(() => {
                  setIsOptimizing(false)
                  setCurrentScreen('results')
                }, 800)
      ...
      .catch(() => {
        // Backend unavailable — use local routing immediately
        clearTimeout(fallbackTimer)
        setPipelineStatus('Using local greedy routing...')
        window.setTimeout(() => {
          setIsOptimizing(false)
          setCurrentScreen('results')
        }, 800)
  ...
  return (
    <main className="app">
      ...
      {isOptimizing && <OptimizationLoading pipelineStatus={pipelineStatus} />}
      {settingsOpen && <SettingsModal onSave={handleSaveSettings} onClose={() => setSettingsOpen(false)} />}
      {routingDetailsOpen && <RoutingDetailsModal onClose={() => setRoutingDetailsOpen(false)} orders={orders} vehicles={vehicles} truckRoutes={truckRoutes} totalDistance={totalDistance} stops={stops} />}
