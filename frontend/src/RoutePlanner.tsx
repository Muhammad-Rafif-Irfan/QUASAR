...
// Mock order data for Screen 02. Each row stays editable in the browser.
...
// Mock fleet data for Screen 02.
const initialVehicles: Vehicle[] = [
  { id: 'truck-1', name: 'Truck 1', capacity: '60' },
  { id: 'truck-2', name: 'Truck 2', capacity: '100' },
]

function RoutePlanner({ onBack, onOpenSettings }: RoutePlannerProps) {
  // Local React state makes the mock rows addable, removable, and editable.
  ...
  // Optimization is deliberately a console-only mock until backend work begins.
  const runOptimization = () => {
    console.log('QUASAR Route Planner mock data', { orders, vehicles })
  }
  ...
  return (
    <div className="planner-shell">
      ...
      <div className="planner-footer-actions"><button type="button" className="settings-button" onClick={onBack}><ArrowLeft size={16} /> Back</button><button type="button" className="route-action" onClick={runOptimization}>Run Optimization</button></div>
      </section>
    </div>
  )
}
